...
Ссылка на полезнеёшую статью: https://habr.com/ru/articles/782608/. Ссылка на репозиторий: https://github.com/TheAlgorithms/JavaScript/tree/master

| Нотация | Определение |
| ------- | ----------- |
| Big O (Ο) | верхняя граница сложности (часто, но не обязательно, используется для худшего сценария) |
| Big Omega (Ω) | нижняя граница сложности (часто, но не обязательно, используется для лучшего сценария) | 
| Big Theta (Θ) |	точная граница, когда Big O = Big Omega (верхняя и нижняя границы совпадают с точностью до константы) |
...
3) Хеш-таблица (Hash Table: Object/Map) - хранит пары ключ-значение. Доступ за O(1). Map - отличительная особеннность структуры данных Map это то, что в Map ключем может быть все что угодно. Сохраняется порядок при добавлении.Методы: set - добавить элемент, get - получить элемент, has - есть ли ключ, delete - удаляем по ключу, clear - очищаем все, size - узнать размер.:
```
// обычный объект
const map = {};
map['key'] = 'value';
